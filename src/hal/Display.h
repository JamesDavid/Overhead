...
public:
  ...
  // Runtime display orientation (LovyanGFX rotation 0..7 = the 4 orientations x mirror) + colour
  // invert, for dialing in different CYD variants (some ship 90 deg off / mirrored) and reflected
  // HUDs. SPI panels apply rotation+invert in hardware; the RGB CrowPanel applies the mirror bit +
  // invert in software at the flush push. mirrored()/rotation() let the touch layer + cal follow.
